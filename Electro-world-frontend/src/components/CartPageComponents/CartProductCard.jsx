...
const CartProductCard = ({id,name,offer_price,actual_price,image,qty,removeAddToCartProduct,quantityUpdateHandler}) => {
  return (
    <Grid  templateColumns='1fr 3fr 1fr' gap={2} style={{ boxShadow: " rgba(9, 30, 66, 0.25) 0px 4px 8px -2px, rgba(9, 30, 66, 0.08) 0px 0px 0px 1px", padding: "1rem", borderRadius: "0.4rem",margin:"1rem" }}>
      <GridItem>
        ...
        <Center>
        <Button colorScheme='red' isDisabled={qty === 1} onClick={()=>quantityUpdateHandler(id,-1)}>-</Button>
        <Box as="span">{qty}</Box>
        <Button colorScheme='red' onClick={()=>quantityUpdateHandler(id,1)}>+</Button>
        </Center>
      </GridItem>
      <GridItem mt="2rem">
        <Heading as='h6' size='xs'>{name}</Heading>
        <Button mt="1rem" colorScheme='red' onClick={()=>removeAddToCartProduct(id)}>Remove</Button>
      </GridItem>
      <GridItem mt="2rem">
