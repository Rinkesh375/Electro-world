...
const Register = () => {
    ...
    const handleUserRegister = (e)=>{
        e.preventDefault();
        axios.get(`${url}/users/${email}`)
        .then((res)=>toast({ title: 'Email already exists.',
        status: 'error',
        duration: 4000,
        isClosable: true,
        position:"top"}))
        ...
        .catch(async()=>{
           ...
           else toast({position:"top",status: 'error',duration: 4000,isClosable: true,description:res.message})
